...
namespace ReversalOfSpirit.Gameplay.Ros.Cards.Effects
{
    public class AbsortPhysicalForCounterAttackEffect : GameEffect
    {
        ...
        public override async UniTask OnEndMagicalAtkTurn(IEffectContext context, IRosGame game, RosRoundPhrase roundPhrase)
        {
            ...
            if (absortValue > 0)
            {
                await context.Owner.Game.ExecuteSequential(new List<GameAction>() { new PhysicalCounterAttackAction(absortValue, game.GetOpponent(context.Owner), context.PlayerSlot) });
                IsExecuted = true;
            }
        }
        ...
        public override async UniTask OnPlayerGetDamage(IEffectContext context, SubHpAction subHpAction, IRosGame game)
        {
            ...
            if (subHpAction.damageType == DamageType.PhysicalDamage)
            {
                ...
                await context.Owner.Game.ExecuteSequential(new List<GameAction>() { new AbsorbPhysicalDamageAction(subHpAction.value, percent, this, context.Owner, context.PlayerSlot.Terriotory, context.PlayerSlot, subHpAction.ActionSequenceIndex + 1) });
            }
        }
