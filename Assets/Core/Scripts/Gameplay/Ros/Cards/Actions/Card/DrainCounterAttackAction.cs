...
namespace ReversalOfSpirit.Gameplay.Ros.Cards.Actions
{
    public class DrainCounterAttackAction : GameAction
    {
        ...
        public override async UniTask Execute(IRosGame game)
        {
            ...
            await game.ExecuteSequential(new List<GameAction> { new SubHpAction(value, target, this.Slot, Enums.DamageType.DrainDamage), new RestoreHpAction(value, game.GetOpponent(target), this.Slot, ActionSequenceIndex + 1) });
        }
